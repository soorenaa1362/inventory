        ...
        <div class="card collapse-icon accordion-icon-rotate"
            ...
            <div id="heading10" class="card-header"
                ...
                <a data-toggle="collapse" href="" data-target="#accordion10"
                    ...
                    style="color:#FFFFFF;font-weight: 100">فرم تعیین انبار</a>
            </div>
            <div id="accordion10" data-parent="#accordionWrap1" 
                aria-labelledby="heading10" class="collapse show">
                <div class="card-body" style="padding: 15px">
                    <div class="px-3">
                        <form class="form" action="{{route('AllocateRepositoryStore' , [$contract->id])}}" 
                            method="post">
                            @csrf
                            <input type="hidden" name="id" value="{{$procuctionStage->id ?? ''}}">
                            <div class="form-body">
                                <div class="row">
                                    <div class="col-xl-6">
                                        <div class="form-group">
                                            <label>اختصاص انبار</label>
                                            <div class="input-group">
                                                <select name="fk_repository" class="form-control"
                                                    value="{{$procuctionStage->fk_repository ?? ''}}">
                                                    @foreach ($allrepositories as $repository)
                                                        <option value="{{$repository->id}}"
                                                            {{ ((isset($procuctionStage)) &&
                                                                $procuctionStage->repository->id == $repository->id) 
                                                                ?
                                                                    ' selected=""'
                                                                :
                                                                    '' 
        ...
        <div class="card-header">
            <div class="card-title-wrap bar-success">
                <div class="row">
                    <div class="col-md-9">
                    <h4 class="card-title">
                        لیست انبارهای قرارداد {{$contract->Piece->name}} {{$contract->Customer->name}}
                    </h4>
                    </div>
                </div>
            </div>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <table class="table table-striped
                                    ...
                                    <tbody>
                                        @foreach ($productionStage as $key => $stage)
                                            <tr role="row" class="odd">
                                                ...
                                                <td style="text-align: center;">
                                                    <a href = "{{route('AllocateContractors' , [$stage->id])}}"
                                                        class="btn btn-sm
                                                        btn-outline-success round mb-0">
                                                        مدیریت
                                                    </a>
                                                    <a href = "{{route('AllocateRepositoryDestroy' , [$stage->id])}}"
                                                        class="btn btn-sm
